...
def startLogin():		
	try:
		...
		if result:
			...
			welcLbl['text'] = 'Chào '+userName+',\nChào mừng đến với\nVKU'
			loginStatus['text'] = 'Đã mở khóa'
			loginStatus['fg'] = 'green'
			faceStatus['text']='(Đăng nhập)'
...
def trainFace():
	...
	print('Cấu hình được tạo thành công!!!')
...
def startCapturing():
	...
	if count==100:
		...
		statusLbl['text'] = '(Khuôn mặt được thêm vào thành công)'
		cap.release()
...
def Add_Face():
	...
	if user_name != '' and user_gender!=0:
		if agr.get()==1:
			...
			statusLbl['text'] = '(Kiểm tra điều kiện.)'
	else:
		statusLbl['text'] = '(Vui lòng điền thông tin chi tiết.)'
...
#welcome label
welcLbl = Label(root1, text='Xin chào,\nChào mừng đến với\nVKU', font=('Arial Bold', 15), fg='#303E54', bg=background)
welcLbl.pack(padx=10, pady=20)

#add face
loginStatus = Label(root1, text='Đã khóa', font=('Arial Bold', 15), bg=background, fg='red')
loginStatus.pack(pady=(40,20))	

if os.path.exists('userData/trainer.yml')==False:
	loginStatus['text'] = 'Khuôn mặt bạn chưa được đăng ký'
	addFace = Button(root1, text='   Đăng ký khuôn mặt   ', font=('Arial', 12), bg='#018384', fg='white', relief=FLAT, command=lambda:raise_frame(root2))
	addFace.pack(ipadx=10)
else:
	Thread(target=startLogin).start()
	
#status of add face
faceStatus = Label(root1, text='(Khuôn mặt không được phát hiện)', font=('Arial 10'), fg=textColor, bg=background)
...
#name
nameLbl = Label(userFrame2, text='Họ và tên', font=('Arial Bold', 12), fg='#303E54', bg=background)
...
genLbl = Label(userFrame2, text='Giới tính', font=('Arial Bold', 12), fg='#303E54', bg=background)
...
agree = ttk.Checkbutton(userFrame2, text='Tôi đồng ý sử dụng khuôn mặt của mình để bảo mật', style='Wild.TCheckbutton', takefocus=False, variable=agr)
agree.place(x=28, y=100)
#add face
addBtn = Button(userFrame2, text='    Thêm khuôn mặt    ', font=('Arial Bold', 12), bg='#01933B', fg='white', command=Add_Face, relief=FLAT)
...
Label(root3, text="Chọn hình đại diện của bạn", font=('arial', 15), bg=background, fg='#303E54').pack()
...
Button(root3, text='         OK         ', font=('Arial Bold', 15), bg='#01933B', fg='white', bd=0, relief=FLAT, command=SuccessfullyRegistered).pack()
...
Label(root4, text="Tài khoản của bạn đã được kích hoạt thành công!!", font=('Arial Bold',15), bg=background, fg='#303E54', wraplength=300).pack(pady=10)
Label(root4, text="Khởi chạy lại ứng dụng để bắt đầu cuộc trò chuyện với trợ lý cá nhân của bạn", font=('arial',13), bg=background, fg='#A3A5AB', wraplength=350).pack()
